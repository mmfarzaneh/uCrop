...
import ir.ucrop.model.CutInfo;
import ir.ucrop.util.FileUtils;
import ir.ucrop.util.MimeType;
import ir.ucrop.util.ScreenUtils;
 ...
 * @describe：多图裁剪
 */
public class PictureMultiCuttingActivity extends ir.ucrop.UCropActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // 自定义裁剪输出名
        renameCropFilename = intent.getStringExtra(ir.ucrop.UCrop.Options.EXTRA_RENAME_CROP_FILENAME);
        // 是否单独拍照
        isCamera = intent.getBooleanExtra(ir.ucrop.UCrop.Options.EXTRA_CAMERA, false);
        // 是否混选模式
        isWithVideoImage = intent.getBooleanExtra(ir.ucrop.UCrop.Options.EXTRA_WITH_VIDEO_IMAGE, false);
        // 裁剪数据
        list = getIntent().getParcelableArrayListExtra(ir.ucrop.UCrop.Options.EXTRA_CUT_CROP);
        // 列表是否显示动画效果
        isAnimation = getIntent().getBooleanExtra(ir.ucrop.UCrop.Options.EXTRA_MULTIPLE_RECYCLERANIMATION, true);
        // Crop cut list
    ...
    private void addPhotoRecyclerView() {
        boolean isMultipleSkipCrop = getIntent().getBooleanExtra(ir.ucrop.UCrop.Options.EXTRA_SKIP_MULTIPLE_CROP, true);
    ...
    protected void resetCutData() {
        ...
        extras.putParcelable(ir.ucrop.UCrop.EXTRA_INPUT_URI, uri);
        ...
        extras.putParcelable(ir.ucrop.UCrop.EXTRA_OUTPUT_URI,
        ...
        try {
            ...
            if (cutIndex >= list.size()) {
                setResult(RESULT_OK, new Intent()
                        .putExtra(ir.ucrop.UCrop.Options.EXTRA_OUTPUT_URI_LIST, list)
                );
                onBackPressed();
            } else {
                resetCutData();
